        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                ...
                <div className="rounded-xl border border-sidebar-border/70 bg-sidebar p-3 sm:p-6 dark:border-sidebar-border">
                    <Form method="post" action="/links" resetOnSuccess className="space-y-4">
                        {({ processing, errors, wasSuccessful }) => (
                            <>
                                <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                                    <div className="grid gap-2 md:col-span-2">
                                        <Label htmlFor="url">URL</Label>
                                        <Input id="url" name="url" type="url" placeholder="https://example.com/article" disabled={processing} />
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    {wasSuccessful && <span className="text-sm text-green-600 dark:text-green-400">Link added successfully!</span>}
                                </div>
                            </>
                        )}
                    </Form>
        ...
        <Collapsible open={isOpen} onOpenChange={setIsOpen}>
            <div className="rounded-lg border border-border bg-background p-4">
                <div className="flex w-full min-w-0 items-start">
                    <CollapsibleTrigger className="-mt-2 flex min-w-0 flex-1 items-start justify-start pt-2 text-left">
                        <div className="flex-1">
                            <div className="mb-1 flex items-center gap-1.5">
                                <h3 className="truncate text-sm">{userLink.link.title}</h3>
                                <a
                            ...
                            {userLink.link.description && (
                                <p className="mt-1 line-clamp-2 text-sm text-muted-foreground">{userLink.link.description}</p>
                            )}
                            {/* Compact Status and Category Controls */}
                            <div className="flex w-full items-center gap-1 md:gap-2" onClick={(e) => e.stopPropagation()}>
                                <Form method="patch" action={`/user-links/${userLink.id}`}>
                                    {({ processing }) => (
                                        <>
                                            <Select
                                                name="status"
                                                ...
                                                onValueChange={(value) => {
                                                    const form = document.querySelector(
                                                        `form[action="/user-links/${userLink.id}"]`,
                                                    ) as HTMLFormElement;
                                        ...
                                        <>
                                            <Select
                                                ...
                                                onValueChange={(value) => {
                                                    ...
                                                    if (categoryForm) {
                                                        const categoryInput = categoryForm.querySelector(
                                                            'input[name="category"]',
                                                        ) as HTMLInputElement;
                            ...
                            {({ processing, errors, wasSuccessful }) => (
                                <div className="space-y-2">
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        {wasSuccessful && <span className="text-sm text-green-600 dark:text-green-400">Note added!</span>}
                                    </div>
                                </div>
                            )}
                        </Form>
